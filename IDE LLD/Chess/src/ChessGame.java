...
public class ChessGame {
    ...
    public ChessGame() {
        board = new Board();
        players = new Player[]{new Player(PieceColor.WHITE), new Player(PieceColor.BLACK)};
        currentPlayer = 0;
    }
    ...
    public void start() {
        ...
        while (true) {
            ...
            if(move == null) {
                System.out.println("Wrong move by Player: " + currentPlayer);
                continue;
            }
            // Make the move on the board
            boolean success = player.makeMove(board, move);
            if (!success) {
                System.out.println("Wrong move by Player: " + currentPlayer);
                continue;
            }
